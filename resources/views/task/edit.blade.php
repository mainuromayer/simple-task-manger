...
@section('content')
    <div class="container mt-5">
        ...
        <div class="row mt-2 mb-3">
            <form action="{{ route('task.update', ['id' => $task->id]) }}" method="post" class="col-12">
                @csrf
                    ...
                    <select name="project" id="project" class="form-control">
                        ...
                        @foreach ($projects as $project)
                            <option value="{{ $project->id }}" 
                                {{ old('project', $task->project_id) == $project->id ? 'selected' : '' }}>
                                {{ $project->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" placeholder="Description" rows="4">{{ $task->description }}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
